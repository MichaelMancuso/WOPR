...
# 2010 - Modified by Mike Piscopo to align risk ratings to a 1-5 scale
# 	 and account for risk factor ratings of "Critical"
#
#
#   tissynbe.py
...
def parse_nbe(nbe):
    ...
    nessus.convert_nbe2csv _at_ tssci-security.com.  I'll update the script to account for 
    ...
    with open(nbe, 'rU') as file:
        ...
        for line in file:
            ...
            if line.startswith("results"):
                ...
                line = line.split('|',7)
                if len(line) >= 7:
			# MP Added check to adjust risk severity based on "Risk factor" text.
			# Find is index-based.  Not present returns -1.
	
		    if len(line) > 7:
			# All 8 fields.  If not all 8 fields, don't bother trying overrides.
			    if line[7].find("Risk factor : Critical") >= 0:
				line[5] = 5
			    elif line[7].find("Risk factor: Critical") >= 0:
				line[5] = 5

			    if line[7].find("Risk factor : Serious") >= 0:
				line[5] = 5
			    elif line[7].find("Risk factor: Serious") >= 0:
				line[5] = 5

			    if line[7].find("Risk factor : High") >= 0:
				line[5] = 4
			    elif line[7].find("Risk factor: High") >= 0:
				line[5] = 4

			    if line[7].find("Risk factor : Failed") >= 0:
				line[5] = 4
			    elif line[7].find("Risk factor: Failed") >= 0:
				line[5] = 4

			    if line[7].find("Risk factor : Medium") >= 0:
				line[5] = 3
			    elif line[7].find("Risk factor: Medium") >= 0:
				line[5] = 3

			    if line[7].find("Risk factor : Unknown") >= 0:
				line[5] = 3
			    elif line[7].find("Risk factor: Unknown") >= 0:
				line[5] = 3

			    if line[7].find("Risk factor : Medium/Low") >= 0:
				line[5] = 3
			    elif line[7].find("Risk factor: Medium/Low") >= 0:
				line[5] = 3

			    if line[7].find("Risk factor : Low") >= 0:
				line[5] = 2
			    elif line[7].find("Risk factor: Low") >= 0:
				line[5] = 2

			    if line[7].find("Risk factor : Low/Medium") >= 0:
				line[5] = 2
			    elif line[7].find("Risk factor: Low/Medium") >= 0:
				line[5] = 2

			    if line[7].find("Risk factor : Info") >= 0:
				line[5] = 1
			    elif line[7].find("Risk factor: Info") >= 0:
				line[5] = 1

			    if line[7].find("Risk factor : None") >= 0:
				line[5] = 1
			    elif line[7].find("Risk factor: None") >= 0:
				line[5] = 1

		    # CPE is a common inventory protocol/notation.  These are in addition
		    # to other server identification entries and are really extraneous.  Just dump (or don't add)
		    if line[1:] not in results:
			    if len(line) > 7:
				    if line[3].find("eneral/CPE-T") < 0 and line[6].find("This script will, if given a userid/password") < 0 and line[7].find("A web server is running on this port") < 0:
				    	results.append(line[1:])
			    else:
				    if line[3].find("eneral/CPE-T") < 0 and line[6].find("This script will, if given a userid/password") < 0:
				    	results.append(line[1:])
        ...
        if problems:
            print """nessus.convert_nbe2csv.py had trouble parsing the following results:"""
            for issue in problems:
                print issue
        if t_problems:
            print """nessus.convert_nbe2csv.py had trouble parsing the following lines:"""
            for line in t_problems:
                print issue
        if plugins:
            print """The following plugins potentially caused an issue during parsing:"""
...
def insert_nbe(results,timestamps,database):
    ...
    http://www.tssci-security.com/upload/nessus.convert_nbe2csv_py/nessusdb.sql
...
if __name__ == "__main__":
    ...
    usage  = """usage: nessus.convert_nbe2csv.py [options] args
nessus.convert_nbe2csv.py -d database -f results.nbe
nessus.convert_nbe2csv.py -d database -o output.csv
nessus.convert_nbe2csv.py -d database -o output.csv --order scriptid --sort desc
nessus.convert_nbe2csv.py -d database -o output.csv --count
nessus.convert_nbe2csv.py -f results.nbe -o output.csv
nessus.convert_nbe2csv.py -f results.nbe -d database -o output.csv"""
