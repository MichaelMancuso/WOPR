...
if [ -e $HOME/.ssh/mpiscopo_wopr.privatekey ]; then
	ssh.pivot.sh -i $HOME/.ssh/mpiscopo_wopr.privatekey --localport=5901 --remoteip=$1 --remoteport=5901 mpiscopo@$WOPR
else
	ssh.pivot.sh --localport=5901 --remoteip=$1 --remoteport=5901 mpiscopo@$WOPR
fi
