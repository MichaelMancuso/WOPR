...
# Took -sC script scan out because it was too "noisy"
	nmap $DONTSCANPARM -n -sV -T3 -O -p $PORTLIST  --scan-delay 2ms --max-retries 1 --host-timeout 60m --data-length 31 --version-intensity 3 -sT $UDPSETTING -oA $BASEDESCRIPTOR $TARGET
else
	# Just scan
	nmap $DONTSCANPARM -n -sV -T3 -O -p $PORTLIST  --scan-delay 2ms --max-retries 1 --host-timeout 60m --data-length 31 --version-intensity 3 -sT $UDPSETTING $TARGET
fi
