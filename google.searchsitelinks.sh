...
ShowUsage() {
	echo "Usage: $0 <domain or site> [file type]"
	echo "$0 will query Google and extract all links returned in a site:<domain or site> search"
	echo "The search can be further focused by specifying a file type."
	echo "<domain or site> could be something like: mydomain.com or portal.mydomain.com"
}
...
if [ $# -gt 1 ]; then
	FILETYPE=$2
	GOOGLEURL=`echo "$GOOGLEURL" | sed "s|sitetoken|$SEARCHDOMAIN+filetype:$FILETYPE|g"`
else	
...
#		SEARCHRESULTS=`wget -nv -O- --user-agent="$USERAGENTSTRING" "$GOOGLEURL&start=$STARTLOC"`
		...
		if [ $? -eq 0 ]; then
			...
			# Google may have started blocking.  Dump what we have
			break
		fi
	done
fi
