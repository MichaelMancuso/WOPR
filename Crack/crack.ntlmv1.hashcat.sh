...
if [ $BRUTEFORCE -eq 0 ]; then
	if [ $USINGGPU -eq 0 ]; then
		...
		$HASHCATEXE -m 5500 -a 3 --pw-min=3 --pw-max=7  $hashFile ?uld
	else
		...
		$HASHCATEXE -m 5500 -a 3 -1 abcdefghijklmnopqrstuvwxyzSACMPBTDREGHKLFNVIOW0123456789\!.-_ `cat $hashFile | head -1`
	fi
else
# 	$HASHCATEXE -m 5600 -a 0 -r /usr/share/hashcat/rules/john.rule `head -1 $hashFile` /opt/wordlists/MikesList.wordlist.txt
	$HASHCATEXE -m 5500 -a 0 $hashFile /opt/wordlists/MikesList.wordlist.txt
fi
